...
public:
  ...
  std::string getLastUser() const { return this->_lastUser.at(this->getFocusedChannel().getId()); }
  void setLastUser(const std::string lastUser) { this->_lastUser[this->getFocusedChannel().getId()] = lastUser; }
...
private:
  ...
  std::map<std::string, std::string> _lastUser;
  int _focusedChat;
  bool _typeFocus;
